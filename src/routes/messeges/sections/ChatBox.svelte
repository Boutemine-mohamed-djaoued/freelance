<script>
  ...
  import { id2, id } from "$lib/stores/Session.js";
    ...
    <div class="py-5 md:p-5 md:m-5 md:bg-white rounded-lg h-full">
      <div class="flex justify-between items-center border-b-2 pb-2">
        <div class="flex">
          ...
          <img class="w-10 h-10 aspect-square rounded-full" src={$chatRoom.user.pfp || "/defaultProfile.svg"} alt="" />
            ...
            <div class="flex">
              ...
              <button on:click={(e) => seeTime(e, i)} class:ml-auto={message.postedByUser._id == id2} class:mr-auto={message.postedByUser._id != id2}>
                ...
                {:else if message.type == "image"}
                  <img
                    class="max-w-[15rem] md:max-w-[30rem] rounded-xl my-2 border-2"
                    class:my-img={message.postedByUser._id == id2}
                    class:his-img={message.postedByUser._id != id2}
                    src={`${message.message}`}
                    alt="" />
                ...
                {:else if message.type == "audio"}
                  <audio class="my-1 max-md:max-w-[15rem]" src={`${message.message}`} controls class:my-audio={message.postedByUser._id == id2} class:his-audio={message.postedByUser._id != id2}
                  ></audio>
                {/if}
              </button>
            </div>
