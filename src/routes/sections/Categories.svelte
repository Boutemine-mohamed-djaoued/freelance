...
<section class="wrapper relative isolate overflow-x-clip max-sm:after:hidden max-md:before:hidden min-h-[25rem]">
				...
				{#if categories}
					{#each categories as categorie}
						<div class="swiper-slide p-5">
							...
							<img class="mx-auto max-md:max-h-[15rem] shadow-lg ring-1 ring-black ring-opacity-5 rounded-lg relative bg-gray-200" src={categorie.url} alt="{categorie.title} illustration image" />
						</div>
					{/each}
				{/if}
			</div>
	...
	<div class="!text-white translate-x-8 translate-y-5 2xl:translate-x-16 xl:translate-y-7 swiper-button-next"></div>
	<div class="!text-white -translate-x-8 translate-y-5 2xl:-translate-x-16 xl:translate-y-7 swiper-button-prev"></div>
</section>
