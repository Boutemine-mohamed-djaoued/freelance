...
<section class="bg-white p-5 pb-0 rounded-xl">
	<div class="flex items-center justify-between">
		...
		<div class="me-3 md:me-7 bg-primary-400 rounded-xl p-1 text-white max-md:text-[0.8em] flex">
			<button class:active={active == 'Posted'} on:click={() => (active = 'Posted')} class="px-1 rounded-s-lg">Posted</button>
			<button class:active={active == 'Active'} on:click={() => (active = 'Active')} class=" border-white px-1">Active</button>
				...
				<button on:click={() => handleJobs(i)} class="text-left flex bg-gray-100 bg-opacity-70 justify-between items-center py-2 ps-2 pe-5 my-1 max-md:me-2 me-5 rounded-2xl hover:bg-primary-300 hover:bg-opacity-10">
					<div class="flex items-center">
						{#if active != 'Posted'}
							...
							<div>
								<p>{job.title}</p>
								<p class="text-gray-500">{job.freelancerDetails.firstName + ' ' + job.freelancerDetails.firstName}</p>
							</div>
						{:else}
							<div>
								<p>{job.title}</p>
