<script>
  ...
  onMount(async function getData() {
    try {
      ...
      let data = res.data.freelancerProfile.freelancer;
      console.log(res);
      loading = false;
    ...
    >
      ...
      <div class="image-container relative mb-3">
        ...
        <img
          src={user.pfp}
          alt=""
          class="image -translate-y-[50%] mx-auto rounded-full"
          on:click={() => {
    ...
    <div class="bg-white p-4 rounded-lg mt-4">
      ...
      {#each prv_inf_array as inf}
        <div class="flex justify-between m-10 text-350">
          ...
          {#if inf.key.includes("Skills") || inf.key.includes("Email")}
          ...
          {:else if inf.key.includes("Phone")}
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            <!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
            <p
              id="phone"
