  ...
  <div class="max-md:h-[20rem] md:h-[16rem] max-md:text-[0.8em] overflow-y-auto flex flex-col justify-start">
    {#if jobsArrays[active]?.length > 0}
      {#each jobsArrays[active] as job, i}
        <button
          on:click={() => handleJobs(i)}
          class="text-left flex bg-gray-100 bg-opacity-70 justify-between items-center py-2 ps-2 pe-5 my-1 max-md:me-2 me-5 rounded-2xl hover:bg-primary-300 hover:bg-opacity-10">
          <div class="flex items-center">
            {#if active != "Posted"}
              <div class="pr-3">
                <img class="w-12 aspect-square rounded-full" src={job.details.photo || "/defaultProfile.svg"} alt="" />
              </div>
              <div>
                <p>{job.title}</p>
