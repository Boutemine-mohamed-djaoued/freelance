<script>
	...
	let jobs = [
		...
		{ path: '/client/createGig', value: 'Create Job' },
	...
	<div class="text-400 flex items-center font-bold bg-off-white">
		...
		<nav navbarVisibility="false" class="max-md:w-0 md:w-full">
			<ul class="font-semibold bg-off-white">
				...
				<li class="jobs">
					<Dropdown on:receiveData={consolData} title={'Work'} options={jobs}></Dropdown>
				</li>
				<li
					on:click={() => {
						handleNavbarVisibility();
		...
		<a href={`/${$user.role}/profile/${$user.id}`} class="ml-auto md:ml-5">
			<img class="w-12 rounded-full aspect-square" src={$user.photo || '/defaultProfile.svg'} alt="" />
		</a>
